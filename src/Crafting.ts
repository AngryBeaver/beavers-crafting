...
import { ComponentResults, Result } from "./Result.js";
import { TestHandler } from "./TestHandler.js";
import { Container } from "./Container.js";
import { dnd5eCopyContent, getActorContentPool } from "./ContainerHandler.js";
...
export class Crafting implements CraftingData {
  ...
  async checkCurrency() {
    await this.result.checkCurrency();
  }

  async payCurrency() {
    await this.result.payCurrency();
  }
  ...
  async processInput() {
    ...
    const moneyList: Component[] = [];

    [this.result._components.required, this.result._components.consumed].forEach(crs => {
      this._processComponentResult(crs).forEach(c => {
        if (c.flags?.[Settings.NAMESPACE]?.subtype === "money") {
          moneyList.push(c);
        } else {
    ...
    try {
      ...
      this.restore.push(...itemChange.delete);
      await this._processMoney(moneyList);
    } catch (e) {
      // @ts-ignore
  ...
  private async _processMoney(moneyList: Component[]) {
    if (moneyList.length > 0) {
      const currencies: Currencies = {};
      moneyList.forEach(c => {
        currencies[c.id] = (currencies[c.id] || 0) + c.quantity;
      });
      // @ts-ignore
      await beaversSystemInterface.actorCurrenciesAdd(this.actor, currencies, Settings.get(Settings.CURRENCY_EXCHANGE));
    }
  }
  ...
  private _processComponentResult(componentResults: ComponentResultsData): Component[] {
    ...
    for (const componentResult of componentResults._data) {
      ...
      const shouldBeProcessed = componentResult.userInteraction === "always" || (componentResult.userInteraction === "onSuccess" && !hasError);
      if (shouldBeProcessed) {
        if (!componentResult.isProcessed) {
          componentList.push(component);
        }
      } else if (componentResult.isProcessed && componentResult.userInteraction === "onSuccess" && hasError) {
        component.quantity = component.quantity * -1;
        if (component.flags?.[Settings.NAMESPACE]?.subtype !== "money") {
          this.restore.forEach(r => {
            if (component.isSame(r)) {
              component = beaversSystemInterface.componentCreate(r);
              component.quantity = componentResult.component.quantity * -1;
            }
          });
        }
        componentList.push(component);
  ...
  async processAll() {
    ...
    [this.result._components.required, this.result._components.consumed, this.result._components.produced].forEach(crs => {
      this._processComponentResult(crs).forEach(c => {
        if (c.flags?.[Settings.NAMESPACE]?.subtype === "money") {
          moneyList.push(c);
        } else {
    ...
    try {
      ...
      Hooks.call(Settings.NAMESPACE + ".processed", {
        recipe: this.recipe,
        result: this.result,
        processed: itemChange,
      });
    } catch (e) {
      // @ts-ignore
      ui.notifications.error(e.message);
  ...
  async _getResultComponents(result: Result): Promise<ComponentData[]> {
    const resultComponents = RecipeCompendium._filterData(this.recipe.output, c => c.type === "Item" || c.type === "item");
